...
$stmt = $db->prepare("SELECT * FROM contact_submissions $whereClause ORDER BY created_at DESC");
$stmt->execute($params);
$contacts = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Get statistics
$totalCount = $db->query("SELECT COUNT(*) as count FROM contact_submissions")->fetch()['count'];
$unreadCount = $db->query("SELECT COUNT(*) as count FROM contact_submissions WHERE status = 'unread'")->fetch()['count'];
$todayCount = $db->query("SELECT COUNT(*) as count FROM contact_submissions WHERE DATE(created_at) = CURDATE()")->fetch()['count'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Contact Leads - Zero What Solar Admin</title>
                ...
                <div class="row align-items-center">
                    <div class="col-md-6">
                        <div class="btn-group" role="group">
                            ...
                            <a href="?filter=unread" class="btn <?php echo $filter === 'unread' ? 'btn-warning' : 'btn-outline-warning'; ?>">
                                Unread <?php if ($unreadCount > 0): ?><span class="badge bg-light text-dark"><?php echo $unreadCount; ?></span><?php endif; ?>
                            </a>
                            <a href="?filter=read" class="btn <?php echo $filter === 'read' ? 'btn-success' : 'btn-outline-success'; ?>">Read</a>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <form method="GET" class="d-flex">
                            ...
                            <input type="text" name="search" class="form-control me-2" placeholder="Search leads..." value="<?php echo htmlspecialchars($search); ?>">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (!empty($contact['monthly_bill'])): ?>
                                    <div class="row text-center bg-primary bg-opacity-10 p-2 rounded">
                                        <div class="col-4">
                                            <small><strong>Monthly Bill</strong><br>₹<?php echo number_format($contact['monthly_bill']); ?></small>
                                        </div>
